...
public class FenceProblem {
    ...
    static void maximumajacaentwithoutarray(int arr[]) {
        int include = 0;
        int exclude = 0;
        for (int i = 0; i < arr.length; i++) {
            if (i == 0) {
                ...
                int xinclude = exclude + arr[i];
                int xexclude = include;
                exclude = xinclude;
                include = xexclude;
            } else {
                int xinclude = exclude + arr[i];
    ...
    static void painthouse(int m[][]) {
        ...
        for (int i = 0; i < m.length; i++) {
            if (i == 0) {
                ...
                int t = Math.max(red, blue) + m[i][0];
                int w = Math.max(green, red) + m[i][1];
                int v = Math.max(blue, red) + m[i][2];
                red = t;
                green = w;
                blue = v;
            }
    ...
    static void fence(int arr[], int idx, int color[], int cx, int fx, String value) {
        if (cx == 3) {
            if (idx == 5) {
        ...
        if (fx < 2) {
            fence(arr, idx + 1, color, cx, fx + 1, value + color[cx]);
            fence(arr, idx + 1, color, cx + 1, 0, value + color[cx]);
        } else if(fx>2) {
            fence(arr, idx + 1, color, cx + 1, 0, value + color[cx]);
        }
    }
    ...
    public static void main(String[] args) {
        fence(new int[5], 0, new int[]{1, 2, 3}, 0, 0, "");
