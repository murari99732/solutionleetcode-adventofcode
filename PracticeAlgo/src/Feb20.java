...
public class Feb20 {
    ...
    public static void main(String... args) throws Exception {
        contigousarray(new int[]{1, 0, 0, 1, 0, 1, 1});
        ArrayList<Cordinate>[] ar = new ArrayList[10];
        findall(ar, 0, new boolean[10], new ArrayList<>());
    }

    static boolean graphcyclic(ArrayList<Cordinate>[] arr, int val, boolean[] b) {
        if (b[val] == true) {
            return true;
        }
        b[val] = true;
        for (Cordinate it : arr[val]) {
            if (b[it.y] == false) {
                boolean a = graphcyclic(arr, it.y, b);
                if (a == true) {
                    return true;
                }
            }
    ...
    static void bfs(ArrayList<Cordinate>[] arrayLists) {

        Queue<Rest> q = new LinkedList<>();
        q.add(new Rest(0, 0 + " "));
        boolean b[] = new boolean[5];
        while (!q.isEmpty()) {
            Rest w = q.poll();
            b[w.idx] = true;
            for (Cordinate it : arrayLists[w]) {
                if (b[it.y] == false) {
                    q.add(new Rest(it.y, ""));
                }
            }
    ...
    static boolean swap(int m[][], int dx, int dy, boolean b[][]) {
        if (dx == 2 && dy == 3) {
            return true;
        }
        b[dx][dy] = true;
        for (int i = 1; i <= 8; i++) {
            int vx = dx + x[i];
            int wy = dy + y[i];
            if (vx >= 0 && wy >= 0 && vx < m.length && wy < m.length && b[vx][wy] == false) {
                boolean bc = swap(m, vx, wy, b);
                if (bc == true) {
                    return true;
                }
            }
    ...
    static void friendspairing(ArrayList<Cordinate>[] ar, boolean b[], ArrayList<Integer> arrayList, int val) {
        arrayList.add(val);
        b[val] = true;
        for (Cordinate it : ar[val]) {
            if (b[it.y] == false) {
                friendspairing(ar, b, arrayList, it.y);
            }
        }
    ...
    static void getconnected(int m[][], int x, int y, ArrayList<Integer> ar, boolean b[]) {

        if (x >= m.length || y >= m.length || x < 0 || y < 0 || m[x][y] == -1 || m[x][y] == 1) {
            return;
        }
        m[x][y] = -1;
        getconnected(m, x + 1, y, ar, b);
        getconnected(m, x, y + 1, ar, b);
        getconnected(m, x + 1, y, ar, b);
        getconnected(m, x - 1, y - 1, ar, b);
    }

    static void findall(ArrayList<Cordinate>[] ar, int val, boolean b[], ArrayList<Integer> arrayList) {
        if (val == 6) {
            arrayList.add(val);
            return;
        }
        ...
        for (Cordinate it : ar[val]) {
            if (b[it.y] == false) {
                findall(ar, it.y, b, arrayList);
            }
        }

    }

    static void bipartion(ArrayList<Cordinate>[] ar) {
        boolean b[] = new boolean[ar.length];
        for (int i = 0; i < ar.length; i++) {
            if (b[i] == false) {
                boolean v = DFS(ar, i, b);
            }
        }
    }

    private static boolean DFS(ArrayList<Cordinate>[] ar, int i, boolean[] b) {
        if (i == 6) {
            return true;
        }
        b[i] = true;
    ...
    static class Rest {
        ...
        public Rest(int idx, String path) {
            this.idx = idx;
            this.path = path;
        }
    }
