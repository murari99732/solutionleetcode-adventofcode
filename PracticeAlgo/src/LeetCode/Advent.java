...
public class Advent {
	...
	static void reporting(ArrayList<ArrayList<Character>> ar) {
		HashMap<Character, Set<Character>> hm = new HashMap<Character, Set<Character>>();

		for (int i = 0; i < ar.size(); i++) {
			ArrayList<Character> a = ar.get(i);
			if (hm.containsKey(a.get(0))) {
				Set<Character> hc = hm.get(a.get(0));
				if (!hc.contains(a.get(1))) {
					Set<Character> dc = hm.get(a.get(1));
					if (dc != null) {
						hc.addAll(dc);
					}
					hc.add(a.get(1));
	...
	static void tour(ArrayList<ArrayList<String>> arr) {
		HashMap<String, ArrayList<String>> map = new HashMap<String, ArrayList<String>>();
		for (ArrayList<String> it : arr) {
			ArrayList<String> bs = new ArrayList<>();
			if (!map.containsKey(it.get(0))) {
				String st = it.get(1);
				if (map.containsKey(st)) {
					ArrayList<String> br = map.get(st);
					bs.addAll(br);
				}
				bs.add(st);
				map.put(it.get(0), bs);
				...
				if (map.containsKey(it.get(1))) {
					ArrayList<String> st = map.get(it.get(1));
					bs.addAll(st);
				}
				bs.add(it.get(1));
				map.put(it.get(0), bs);
	...
	static void destination(ArrayList<ArrayList<String>> ar) {
		HashMap<String, Boolean> hashmap = new HashMap<String, Boolean>();
		for (int i = 0; i < ar.size(); i++) {
			ArrayList<String> arr = ar.get(i);
	...
	static void getpair(int arr[], int k) {
		...
		for (Integer m : hashmap.keySet()) {
			if (hashmap.get(m) != -1) {
				int val = hashmap.get(m);
				int dal = hashmap.get(k - m);
				int v = val - dal;
				if (v == 0) {
					d = d + val;
					hashmap.put(m, -1);
	...
	static void findentries(int arr[]) {
		Set<Integer> set = new HashSet<Integer>();
		for (int i = 0; i < arr.length; i++) {
			set.add(arr[i]);
		}
		...
		System.out.println(f + "   " + dc + " " + rf);
		System.out.println(f * dc * rf);
	}

	static void getpassword(String a) {

	}

	private static void getpassword(String[] a) {
		int count = 0;
		for (int i = 0; i < a.length; i++) {
			boolean flag = parse(a[i]);
			if (flag == true) {
				count++;
			}
		}
	...
	static boolean parse(String a) {
		...
		for (int i = 0; i < a.length(); i++) {
			if (a.charAt(i) == '-') {
				if (i >= 2 && Character.isDigit(a.charAt(i - 2))) {
					sval = Integer.parseInt(a.substring(i - 2, i));
				} else {
					sval = Integer.parseInt(Character.toString(a.charAt(i - 1)));
				}
				if (Character.isDigit(a.charAt(i + 2))) {
					eval = Integer.parseInt(a.substring(i + 1, i + 3));
				} else {
					eval = Integer.parseInt(Character.toString(a.charAt(i + 1)));
				}

			}

			if (a.charAt(i) == ':') {
				charToSearch = a.charAt(i - 1);
				dc = a.substring(i + 1, a.length());
			}
		}
		dc = dc.trim();
		int index = 0;
		boolean flag = false;
		boolean slag = false;
		for (int i = 0, f = 1; i < dc.length(); i++, f++) {

			if (f == sval) {
				if (dc.charAt(i) == charToSearch) {
					flag = true;
				}
			}
			if (f == eval) {
	...
	public static void main(String[] args) {
		String a[] = { "eyr:2027", "hcl:#602927", "hgt:186cm byr:1939 iyr:2019 pid:552194973 ecl:hzl", "",
	...
	private static void parsevalue(String[] a) {
		...
		for (int i = 0; i < a.length; i++) {
			String dc = a[i];
			if (dc.trim().isEmpty()) {
				boolean flag = getvalue(ab, set);
				if (flag == true) {
					boolean fg = validate(ab, set);
					if (fg == true)
					{
	...
	private static boolean validate(String ab, ArrayList<String> set) {

		Set<String> eyecolour = new HashSet<String>();
		eyecolour.add("amb");
		eyecolour.add("blu");
		eyecolour.add("brn");
		eyecolour.add("gry");
		...
		for (int i = 0; i < bc.length; i++) {
			if (bc[i].contains("byr")) {
				String check = bc[i + 1].trim();
				String check2 = "";
				for (int l = 0; l < check.length(); l++) {
					if (check.length() == l || check.charAt(l) == ' ') {
						break;
					}
					check2 = check2 + check.charAt(l);
				}
				int val = 0;
				try {
					val = Integer.parseInt(check2);
			...
			} else if (bc[i].contains("hgt")) {

				if (bc[i + 1].contains("cm")) {

					String check2 = "";
					for (int l = 0; l < bc[i + 1].length(); l++) {
						if (bc[i + 1].length() == l || bc[i + 1].charAt(l) == ' ') {
							break;
						}
						check2 = check2 + bc[i + 1].charAt(l);
					}
					String check = check2.trim().substring(0, check2.length() - 2);
					int val = 0;
					try {
						val = Integer.parseInt(check);
					} catch (NumberFormatException e) {
						return false;
					}
					if (val < 150) {
				...
				if (check2.charAt(0) != '#') {
					return false;
				} else if (check2.length() != 7) {
					return false;
				}
				for (int j = 1; j < check2.length(); j++) {

					if (!Character.isDigit(check2.charAt(j)) && check2.charAt(j) > 'f') {
						return false;
					}

				}
			} else if (bc[i].contains("ecl")) {
				String check = bc[i + 1].trim();
				String check2 = "";
				for (int l = 0; l < check.length(); l++) {
					if (check.length() == l || check.charAt(l) == ' ') {
				...
				if (!eyecolour.contains(check2)) {
					return false;
				}
			} else if (bc[i].contains("pid")) {
				String check = bc[i + 1].trim();
				String check2 = "";
				for (int l = 0; l < check.length(); l++) {
					if (check.length() == l || check.charAt(l) == ' ') {
...
//				} else if (Integer.parseInt(Character.toString(check2.charAt(0))) != 0) {
//					return false;
//				}
				try {
					Integer.parseInt(check2);
				} catch (NumberFormatException e) {
					return false;
				}
			}
	...
	private static boolean getvalue(String ab, ArrayList<String> set) {

		int d = 0;
		for (int i = 0; i < set.size(); i++) {
			if (ab.contains(set.get(i))) {
				d++;
			}
		}
		if (d == 7) {
	...
	static void day5(String a[]) {
		int sum = 0;
		String ab = "";
		for (int i = 0; i < a.length; i++) {
			String dc = a[i];
			if (dc.trim().isEmpty()) {
				int value = getvalue(ab);
				sum = sum + value;
				ab = "";
				System.out.println();
			} else
	...
	private static void parse(String dc, HashMap<Character, Integer> set) {
		for (int i = 0; i < dc.length(); i++) {
			set.put(dc.charAt(i), set.getOrDefault(dc.charAt(i), 0) + 1);
		}
