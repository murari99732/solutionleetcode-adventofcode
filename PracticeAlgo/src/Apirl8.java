...
public class Apirl8 {
    ...
    static void substring(String m) {
        ...
        for (int i = 0; i < m.length(); i++) {
            for (int j = i; j < m.length(); j++) {
                if (i == j) {
                    ...
                    if (m.charAt(i) == m.charAt(j) && dp[i - 1][j - 1])
                        dp[i][j] = true;
                }
            }
        }
    ...
    static void maximumnon(int arr[]) {
        Bridge b[] = new Bridge[arr.length];
        Arrays.stream(b).sorted((a, c) -> a.dx - c.dx);
    ...
    static void longestvel(int arr[]) {
    ...
    static void paintfence(int arr[]) {
        int a = 0;
        int b = 0;
        int c = 0;
